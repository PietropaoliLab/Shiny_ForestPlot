...
get_custom_theme <- function(font_size, legend_position) {
...
server <- function(input, output, session) {
  ...
  create_plot <- function(data) {
    p <- ggplot(data, aes(x = log2FoldChange, y = Comparison, color = Comparison)) +
      ...
      get_custom_theme(input$font_size, input$legend_position)
    
    if (input$show_pvalue) {
      p <- p + geom_text(aes(x = log2FoldChange*1.1, y = Comparison, label = paste0("P = ", padj)), size = 3, colour = "black", hjust = -0.5)
    }
